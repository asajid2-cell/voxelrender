...
if ($ElevateY -ne 0) { $env:VENPOD_SPARSE_WALK_TEST_EYE_OFFSET_Y = "$ElevateY"; $env:VENPOD_SPARSE_WALK_TEST_FLY = "1" }
if ($FullRes) { $env:VENPOD_RAYMARCH_RENDER_SCALE = "1.0" }
if ($RealAim) { $env:VENPOD_SPARSE_BRUSH_SMOKE_REAL_AIM = "1" }
if ($PaintStone) { $env:VENPOD_SPARSE_BRUSH_SMOKE_PAINT_STONE = "1" }
# Case 2 is erase: hold the RIGHT button so it takes the real erase path
# (IsErasing -> buildStroke false -> ray-follow), not the paint climb.
if ($Case -eq 2) { $env:VENPOD_SPARSE_BRUSH_SMOKE_ERASE = "1" }
...
foreach ($v in @(
    ...
    "VENPOD_SPARSE_WALK_TEST_EYE_OFFSET_Y","VENPOD_RAYMARCH_RENDER_SCALE","VENPOD_SPARSE_WALK_TEST_FLY",
    "VENPOD_SPARSE_BRUSH_SMOKE_FOLLOW_CAMERA","VENPOD_SPARSE_BRUSH_SMOKE_FOLLOW_DISTANCE","VENPOD_SPARSE_BRUSH_SMOKE_REAL_AIM","VENPOD_SPARSE_BRUSH_SMOKE_PAINT_STONE","VENPOD_SPARSE_BRUSH_SMOKE_ERASE",
    ...
    "VENPOD_CAPTURE_HIDE_UI","VENPOD_CAPTURE_DIR","VENPOD_CAPTURE_START_FRAME",
