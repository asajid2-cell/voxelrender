...
struct ChunkCoord {
    ...
    static uint32_t LocalCoord(int32_t worldCoord, uint32_t chunkSize) {
        const int32_t size = static_cast<int32_t>(chunkSize);
        int32_t local = worldCoord % size;
        return static_cast<uint32_t>(local < 0 ? local + size : local);
    }
